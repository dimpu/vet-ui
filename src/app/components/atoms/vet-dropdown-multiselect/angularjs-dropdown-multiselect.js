...
function ngDropdownMultiselectDirective($window, $filter, $document, $compile, $parse, $timeout) {
    return {
        restrict: 'AE',
        scope: {
            selectedModel: '=',
        ...
        template: function(element, attrs) {
            ...
            template += `<ul ng-if="open" class="vet-multiselect-dropdown-menu dropdown-menu dropdown-menu-form"  ng-style="{display: open ? \'block\' : \'none\', height : settings.scrollable ? settings.scrollableHeight : \'auto\' }" >`;
        ...
        link: function($scope, $element, $attrs) {
                ...
                $document.on('click', function(e) {
                    ...
                    if ($scope.open) {
                        ...
                        while (angular.isDefined(target) && target !== null && !parentFound) {
                            ...
                            if (!!target.className.split && contains(target.className.split(' '), 'vet-multiselect-dropdown-menu') && !parentFound) {
            ...
            function position() {
                var offset = {};
                $dropdown = $element.find('ul');
                var boundingClientRect = $dropdownTrigger.getBoundingClientRect();

                offset.top = boundingClientRect.top +
                    ($window.pageYOffset || $document[0].documentElement.scrollTop);
                offset.left = boundingClientRect.left +
                    ($window.pageXOffset || $document[0].documentElement.scrollLeft);

                if ($dropdown[0]) {
                    $dropdown[0].style.position = 'absolute';
                    $dropdown[0].style.overflow = 'visible !important';
                    $dropdown[0].style.left = offset.left + 'px';
                    $dropdown[0].style.top = offset.top + 40 + 'px';
                }
            }
...
const ngDropdownMultiselect =  ['$window', '$filter', '$document', '$compile', '$parse', '$timeout', ngDropdownMultiselectDirective];

const dmDropdownStaticInclude =  ['$compile', function($compile) {
...
export default angular.module('angularjsDropdownMultiselect', [])
    .directive('ngDropdownMultiselect',  ngDropdownMultiselect)
    .directive('dmDropdownStaticInclude',  dmDropdownStaticInclude)
    .name;
